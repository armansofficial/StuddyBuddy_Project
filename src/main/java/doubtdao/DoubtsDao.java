...
public class DoubtsDao  implements DoubtDao{
	...
	public int insertDoubtsIntoDb(QuestionPostEntity obj) {
		String query = "INSERT INTO  user_question_post(title, detail_problem, expecting, tags) "
				+ "VALUES (?, ?, ?, ?)";
		int status = this.jdbcTemplate.update(query, obj.getTitle(), obj.getDetails(), obj.getExpectation(), obj.getTags());
		return status;
	}
	...
	public void getTotalLike(int id) {
		String query="UPDATE user_question_post SET likes=likes+1 WHERE id=?";
	...
	public void getTotalDisLike(int id) {
		String query="UPDATE user_question_post SET dislike=dislike+1 WHERE id=?";
	...
	public int inserStudentData(SmSignUp object) {
		...
		int i=	this.jdbcTemplate.update(query, object.getFname(), object.getLname(), object.getEmail(), object.getMob(), object.getPassword());
		return i;
	}
 ...
 public SmSignUp getUserDataByLoginDetail(String email, String password) {
		...
		String query="SELECT id, fname, email, mob, created_at, lname FROM smuserdata WHERE email=? && password =?";
	...
	public List<AllCourses>  getAllCourses() {
		...
		String query="SELECT * FROM coursce";
...
AllCourses object=new AllCourses();
	String query="SELECT * FROM coursce WHERE id=?";
...
public int postCoursesByAdmin(AllCourses object) {
	String query="INSERT INTO coursce(cname, cdescription, cinstructor, cexperiance, instructorinfo, img, price) VALUES (?, ?, ?, ?, ?, ?, ?)";
	 int status= this.jdbcTemplate.update(query, object.getcName(), object.getcDescription(), object.getcInstructor(), object.getcExperience(), object.getInstructorInfo(), object.getImg(), object.getPrice());
	return status;
}
...
public int deleteCourceById(int id) {
	String query="delete from coursce where id=?";
 int i=	this.jdbcTemplate.update(query, id);
...
public int updateCourceById(AllCourses object) {
	String query="UPDATE coursce SET cname=?, cdescription=?, postedate=?, cinstructor=?, cexperiance=?, instructorinfo=?, img=?, price=? WHERE id=?";
 int status=	jdbcTemplate.update(query,object.getcName(), object.getcDescription(), object.getcPost(), object.getcInstructor(), object.getcExperience(), object.getInstructorInfo(), object.getImg(), object.getPrice(), object.getId());
...
public int addStudentIdFromCourceById(int buyId, int courceId) {
	...
	String query="UPDATE coursce SET buyid=? WHERE id=?";
