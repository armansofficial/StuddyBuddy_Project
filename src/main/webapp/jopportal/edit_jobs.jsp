...
<body>
	...
	int id = Integer.parseInt(request.getParameter("id"));
      JobsDao editJobs=new JobsDao(DbConnection.getConnection());
	Jobs viewjob = editJobs.editjob(id);
	...
	<form action=http://localhost:8879/ST/updateJobs method="post">
		<label for="jobTitle">Job Title:</label> <input type="text"
			id="jobTitle" name="jobTitle" required
			value="<%=viewjob.getjobTittle()%>"><br>
		<br> <label for="jobLocation">Location:</label> <select
			id="joblocation" name="joblocation">
			<option selected value="<%=viewjob.getLocation()%>"><%=viewjob.getLocation()%></option>
		...
		<br> <label for="jobCategory">Role:</label> <select
			id="jobCategory" name="jobCategory">
			<option selected value="<%=viewjob.getRole()%>"><%=viewjob.getRole()%></option>
		...
		<br> <label for="Exprience">Experience:</label> <select
			id="Exprience" name="Exprience">
			<option selected><%=viewjob.getExprience()%></option>
		...
		<br> <label for="jobStatus">Status:</label> <input type="radio"
			id="jobStatusOpen" name="jobStatus" value="Open" checked
			value="<%=viewjob.getStatus()%>"> <label for="jobStatusOpen">Open</label>
		<input type="radio" id="jobStatusClosed" name="jobStatus"
			value="Closed" checked value="<%=viewjob.getStatus()%>"> <label
			for="jobStatusClosed">Closed</label><br>
		<br> <label for="jobDescription">Description:</label><br>
		<textarea id="jobDescription" name="jobDescription" rows="4" cols="50"
			required><%=viewjob.getDescription()%></textarea>
		<br>
		<br> <input type="submit" value="Update Job">
	</form>
    </body>
</html>
